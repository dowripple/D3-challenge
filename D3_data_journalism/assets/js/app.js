...
// object for the axis labels (both x and y)
var labelTitles = { 
  'healthcare': '% Lacking Healthcare',
  'obesity': '% Obese',
  'smokes': '% Smokers',
  'age': 'Median Age',
...
// padding to be used for both axis
var axisPadding = 20;

// function used for xy-scale var upon click on axis label text
function scale(scatterData, chosenAxis, xy) {

    if (xy === 'x') {
      var axisRange = [0, chartWidth]
    } else {
      var axisRange = [chartHeight, 0]
    }
    
    // create scales for chosen axis
    var linearScale = d3.scaleLinear()
      .domain([d3.min(scatterData, d => d[chosenAxis]) * 0.8,
        d3.max(scatterData, d => d[chosenAxis]) * 1.2
      ])
      .range(axisRange);
...
function renderAxis(newScale, axis, xy) {
    ...
    // based on axis passed in, set the scale
    if (xy === 'x') {
      var posAxis = d3.axisBottom(newScale);
    } else {
      var posAxis = d3.axisLeft(newScale);
    }
